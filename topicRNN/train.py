...
def topicRNN_loss(encoder, decoder, doc_batch, tau, V, B):
    # print('***************************************')
...
def sparseTopicRNN_loss(encoder, decoder, doc_batch):
    mus, log_sigmas = encoder(doc_batch,device)
    samples = utils.reparameterize(mus, log_sigmas)
    # print('\t\tmus.shape = ', mus.shape,' log_sigma.shape = ', log_sigmas.shape)
    thetas = samples.softmax(dim = 1)
...
def main(data_file, num_topics):
    ...
    decoder = models.topicRNN(hidden_dim, embedding_dim, num_words, batch_size)
    ...
    for e in range(num_epoches):
        ...
        for batch in batches:
            ...
            #loss += sparseTopicRNN_loss(encoder,decoder,doc_batch)
            optimizer.zero_grad()
            loss = topicRNN_loss(encoder, decoder, doc_batch, tau, V, B)
            epoch_loss += loss
            loss /= batch_size
            loss.backward()
            optimizer.step()
